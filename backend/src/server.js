...
async function updateResource(req, res) {
  const updateOptions = req.method === 'PATCH' ? { new: true, runValidators: true } : undefined;

  try {
    const normalizedPayload = normalizeResourcePayload(req.body);
    ...
    if (req.method === 'PUT') {
      ...
      resource.name = normalizedPayload.name;
      resource.capacity_hours = normalizedPayload.capacity_hours;
      resource.is_active = normalizedPayload.is_active;
    ...
    const updatedResource = await Resource.findByIdAndUpdate(req.params.id, normalizedPayload, updateOptions);
...
app.patch('/api/resources/:id', updateResource);
